...
sentiment_save_path = configenv['sentiment_path']
...
try:
    ...
    model_save_path = sentiment_save_path
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32

    model_l = AutoModelForSequenceClassification.from_pretrained(model_save_path, torch_dtype=torch_dtype).to(device)
    logger.Logg("Model loaded successfully.")

    # Load the processor from local path
    processor_save_path =  sentiment_save_path
    processor_l = AutoTokenizer.from_pretrained(processor_save_path)
    logger.Logg("Processor loaded successfully.")

    sentiment_analysis = pipeline("sentiment-analysis", framework="pt", model=model_l, tokenizer=processor_l)
...
async def transcribe():
    ...
    try:
        ...
        e_time = time.time()
        logger.Logg(f"Transcription completed: {transcription}")
